...
if ( have_posts() ) {
	...
	while ( have_posts() ) {
		...
		$class_array = array( 'btArticleListItem', 'animate', 'gutter' );
	...
	if ( is_search() ) { ?>
		<article class="btNoSearchResults boldSection gutter bottomSemiSpaced topSemiSpaced ">
			<div class="port">
			<?php echo boldthemes_get_heading_html( '', esc_html__( 'We are sorry, no results for: ', 'celebration' ) . get_search_query(), "<a href='" . site_url() . "'>" . esc_html__( 'Back to homepage', 'celebration' )."</a>", 'medium', '', '', '' ) ?>
			</div>
		</article>
	<?php }
}
