...
//				$( this ).css( 'height', $( this ).width() * 9 / 16 );
	...
	$( window ).on( 'btload', function() {
		...
		// gravity form scroll
		if ( $(location).attr('hash').indexOf("#gf_") > -1 ){
			var id_hash = $(location).attr('hash');
			if ( $( id_hash ).length > 0 ) {
				var topOffset = $( id_hash ).offset().top + 100;
				if ( window.btStickyEnabled && topOffset > window.btStickyOffset ) {
					topOffset -= $( '.mainHeader' ).height();
				}
				$( 'html, body' ).animate({ scrollTop: topOffset }, 50);
			}
		}
	});
	...
	$( document ).ready(function() {
		...
		$( document ).on( 'click', 'a', function() {
			if ( ! $( this ).hasClass( 'lightbox' ) && ! $( this ).hasClass( 'add_to_cart_button' ) && ! $( this ).parent().hasClass( 'woocommerce-product-gallery__image' ) ) {
