...
add_action('admin_enqueue_scripts', function ($hook) {
    wp_enqueue_script('hscf-admin-script', plugin_dir_url(__FILE__) . 'js/hscf-admin.js', ['jquery'], '2.28', true);
    ...
    if ($hook === 'toplevel_page_HitchStream_Cloudflare') {
        wp_enqueue_style('hscf-admin-style', plugin_dir_url(__FILE__) . 'css/hscf-admin.css', ['dashicons'], '2.28');
    }
});

add_action('wp_enqueue_scripts', function () {
    if (current_user_can('manage_options')) {
        wp_enqueue_script('hscf-admin-script-frontend', plugin_dir_url(__FILE__) . 'js/hscf-admin.js', ['jquery'], '2.28', true);
