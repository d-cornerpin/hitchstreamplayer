...
class SettingsPage {
    ...
    public static function renderAdmin(): void {
        ...
        if (isset($_GET['delete_input']) && isset($_GET['confirm_delete']) && $_GET['confirm_delete'] === 'yes') {
            (new \HS\Services\LiveInputService())->delete(sanitize_text_field($_GET['delete_input']));
        }
        if (isset($_POST['create_stream']) && !empty($_POST['stream_name'])) {
            (new \HS\Services\LiveInputService())->create(sanitize_text_field($_POST['stream_name']));
        }
        ...
        <div class="wrap">
            ...
            <div class="top-container">
                ...
                <div class="middle-img-container">
                    <div class="middleitem">
                        <a href="https://solo.liveu.tv/dashboard" target="_blank"><img src="https://hitchstream.com/wp-content/uploads/2024/03/livedashboard.png" alt="LiveU Dashboard" /></a>
                    </div>
                    <div class="middleitem">
                        <a href="https://streamer1.hitchstream.com" target="_blank"><img src="https://hitchstream.com/wp-content/uploads/2024/04/placeholderstreamer.png" alt="Placeholder Streamer" /></a>
                    </div>
                    <div class="middleitem">
                        <a href="https://hitchstream.com/control" target="_blank"><img src="https://hitchstream.com/wp-content/uploads/2024/04/controller.png" alt="HitchStream Controller" /></a>
                    </div>
                    <div class="middleitem">
                        <a href="https://dash.cloudflare.com/83584468f508b713b66fd41d85e58280/stream/videos" target="_blank"><img src="https://hitchstream.com/wp-content/uploads/2024/03/cloudflaredashboard.png" alt="CloudFlare Dashboard" /></a>
                    </div>
                </div>
                <div class="form-container right">
                    <form method="post" action="options.php">
                        <?php settings_fields('HSCF_settings'); do_settings_sections('HitchStream_Cloudflare'); submit_button(); ?>
                    </form>
                </div>
            </div>
            ...
            <?php self::renderStreams(); ?>
        </div>
        <?php
    }

    /** Render the current streams list and uploaded videos. */
    private static function renderStreams(): void {
        ?>
        <style>
            .FileSelector { width: 250px; }
            .cfmodal { display:none; position:fixed; z-index:1; left:0; top:0; width:100%; height:100%; overflow:auto; background-color:rgba(0,0,0,0.4); }
            ...
            .middleitem { padding:0px 10px; }
            .form-container.right { align-items:flex-end; }
            .form-table th, .form-table td { padding:5px; vertical-align:middle; }
            ...
            .copy-btn { background:#2271b1; border-color:#2271b1; color:#fff; text-shadow:none; font-size:13px; line-height:2.15384615; min-height:30px; margin:5px; padding:0 10px; cursor:pointer; border-width:1px; border-style:solid; -webkit-appearance:none; border-radius:3px; white-space:nowrap; box-sizing:border-box; }
            .switch { position:relative; display:inline-block; width:30px; height:17px; }
            .switch input { opacity:0; width:0; height:0; }
            .slider { position:absolute; cursor:pointer; top:0; left:0; right:0; bottom:0; background-color:#ccc; transition:.4s; }
            .slider:before { position:absolute; content:""; height:17px; width:16px; left:0px; bottom:0px; background-color:white; transition:.4s; }
        ...
        $liveInputs = (new \HS\Services\LiveInputService())->listWithDetails();
        if (is_string($liveInputs) || isset($liveInputs['error'])) {
            echo '<p>' . esc_html(is_string($liveInputs) ? $liveInputs : $liveInputs['error']) . '</p>';
            return;
        }

        foreach ($liveInputs as $input) {
            ...
            echo "<strong>" . esc_html($input_name) . "</strong> | <span id='status-" . esc_attr($input->uid) . "' class='" . $status_class . "'>" . esc_html($input->status_details ?? 'Status Unavailable') . '</span>';
            ...
            echo '<label class="switch"><br><input type="checkbox" class="stream-toggle" id="streamToggle-' . esc_attr($input->uid) . '" data-rtmp-key="' . $rtmpKey . '"><span class="slider round"></span></label>';
            echo '</div>';
            ...
            $outputs = (new \HS\Services\LiveInputService())->getOutputs($input->uid);
            if (is_array($outputs)) {
                foreach ($outputs as $output) {
                    echo "<div>";
                    if (strpos($output->url ?? '', "youtube") !== false) echo "YouTube ";
                    elseif (strpos($output->url ?? '', "facebook") !== false) echo "Facebook ";
                    else echo esc_html($output->url ?? '');
                    $checked = ($output->enabled ?? false) ? "checked" : "";
                    echo '<label class="switch"><input type="checkbox" class="output-toggle" data-output-id="' . esc_attr($output->uid ?? '') . '" data-input-id="' . esc_attr($input->uid) . '" ' . $checked . '><span class="slider round"></span></label> ';
                    echo '<a href="#" class="delete-output-link" data-output-id="' . esc_attr($output->uid ?? '') . '" data-input-id="' . esc_attr($input->uid) . '" title="Delete Output"><img src="https://hitchstream.com/wp-content/uploads/2024/01/delete_icon.png" width="18px" height="18px" alt="Delete Output" style="vertical-align:middle;"></a>';
                    echo "</div>";
                }
            }
            echo "</div>";

            // Middle Div 2 for Recordings
            echo '<div class="live-input-middle-2">';
            echo "<br><br><strong>Recordings (" . esc_html($input_name) . "):</strong><br>";
            $filteredVideos = (new \HS\Services\RecordingsService())->findByStreamName($input_name);
            if (is_array($filteredVideos) && !empty($filteredVideos)) {
                foreach ($filteredVideos as $video) {
                    ...
                    echo '<a href="' . $downloadLink . '" class="create-download-link" data-video-id="' . esc_attr($video['uid'] ?? '') . '" style="margin-right:10px;"><img src="' . $iconUrl . '" title="' . $downloadTitle . '"></a>';
                    echo '<a href="#" class="generate-videoid" data-video-id="' . esc_attr($video['uid'] ?? '') . '" style="margin-right:10px;"><img src="' . $embedIconUrl . '" title="Get Video ID"></a>';
                    echo '<a href="#" class="delete-video-link" data-video-id="' . esc_attr($video['uid'] ?? '') . '"><img src="' . $deleteIconUrl . '" title="Delete Video"></a>';
                    echo "</div>";
                }
            } else {
            ...
            echo '<iframe src="https://hitchstream.com/player?live=true&inputId=' . esc_attr($input->uid ?? '') . '" style="border:none;width:100%;aspect-ratio:16/9;" allow="fullscreen;accelerometer;gyroscope;autoplay;encrypted-media;picture-in-picture" allowfullscreen></iframe>';
            echo "</div></div>";
            echo "</div>";
        }
        ...
        <div id="Videos Panel">
            ...
            <div id="videos-panel">
                <?php
                $videos = (new \HS\Services\LiveInputService())->listWithDetails(); // Reuse for video listing
                if (is_array($videos)) {
                    foreach ($videos as $video) {
                        ...
                        echo '<iframe src="https://hitchstream.com/player?live=false&autoplay=false&inputId=' . esc_attr($video['uid'] ?? '') . '" style="border:none;width:100%;aspect-ratio:16/9;" allow="fullscreen;accelerometer;gyroscope;encrypted-media;picture-in-picture" allowfullscreen></iframe>';
                        echo '<div class="video-buttons">';
                        echo '<a href="' . $downloadLink . '" class="create-download-link" data-video-id="' . esc_attr($video['uid'] ?? '') . '" style="margin-right:10px;"><img src="' . $iconUrl . '" title="' . $downloadTitle . '"></a>';
                        echo '<a href="#" class="generate-embed-link" data-video-id="' . esc_attr($video['uid'] ?? '') . '" style="margin-right:10px;"><img src="' . $embedIconUrl . '" title="Get Embed Code"></a>';
                        echo '<a href="#" class="delete-video-link" data-video-id="' . esc_attr($video['uid'] ?? '') . '"><img src="' . $deleteIconUrl . '" title="Delete Video"></a>';
                        echo '</div></div>';
                    }
                }
